...
import GqlRepositoryRow from './GqlRepositoryRow';
import RepositoryRowSkeleton from './RepositoryRowSkeleton';

// Component
export default function GqlRepositoryTable({ className, data, onLoadMore }: RepositoryTableProps) {
...
const repositoryRow: RowFn<RepositoryTableFragment> = ({ data, index }) => {
  ...
  if (item) {
    return <GqlRepositoryRow key={item.id} data={item} index={index} />;
  }
